# script to run single-cell Poisson mixed effects eQTL model 
...
for(i in 1:nrow(geneSNPs)){
	...
	full_model <- lme4::glmer(formula = E ~ G + (1 | IND) + (1 | B) + D + AGE + SEX + nUMI + MT + PC1 + PC2 + PC3 + expPC1 + expPC2 + expPC3 + expPC4 + expPC5 + cov + G*cov, 
	                          family = "poisson", nAGQ = 0, data = data, control = glmerControl(optimizer = "nloptwrap"))
	null_model <- lme4::glmer(formula = E ~ G + (1 | IND) + (1 | B) + D + AGE + SEX + nUMI + MT + PC1 + PC2 + PC3 + expPC1 + expPC2 + expPC3 + expPC4 + expPC5 + cov, 
	                          family = "poisson", nAGQ = 0, data = data, control = glmerControl(optimizer = "nloptwrap"))
