...
permutations_dir <- paste0("inputs/",folder,"/severity_permutations/")
...
for (i in 1:length(celltypes)){
	...
	length(which(colnames(residuals)!=rownames(md_i)))
	length(which(colnames(weights)!=rownames(md_i)))
	length(which(colnames(residuals)!=colnames(weights)))

	## model infection differential expression
	design = model.matrix(~ Dataset + Sex + Age_scale + BMI_scale + Ancestry_UMAP + EC_Chronic_renal_insufficiency + EC_Chronic_Heart_Insufficiency + EC_Chronic_Lung_Insufficiency + EC_Chronic_liver_insufficiency + EC_Hypertension + EC_Dyslipidemia + EC_DB1 + EC_DB2 + counts + RespSupp_Sampling, data = md_i)
	...
	## collect outputs
	betas = as.data.frame(vfit$coefficients[, which(colnames(vfit$coefficients) %in% c("RespSupp_Sampling"))]); colnames(betas)[1] <- "betas"
	p_values = as.data.frame(vfit$p.value[, which(colnames(vfit$coefficients) %in% c("RespSupp_Sampling"))]); colnames(p_values)[1] <- "pvalues"
	fdrs = as.data.frame(p.adjust(p_values[,1], method = "BH")); colnames(fdrs)[1] <- "fdrs"
	t_stats = as.data.frame(cbind(rownames(topTable(vfit, coef = "RespSupp_Sampling", number = Inf)), topTable(vfit, coef = "RespSupp_Sampling", number = Inf)$t))
	...
	perm_fdrs = permFDR(full_data = results, full_column_id = "pvalues", perm_data = perms, perm_column_ids = "all", output_name = paste0("outputs/",folder,"/3_severity/"))
	write.table(perm_fdrs$fdrs, file = paste0("outputs/",folder,"/3_severity/",cell_type_i,"_results_with_qvalues.txt"), quote = FALSE, sep = ",")
	...
	corr_fdrs <- subset(perm_fdrs$fdrs)
  	...
  	if(i == 1){
  		...
  		numDEgenes_10[i] <- length(which(corr_fdrs[,"Fdr_SAB"] < 0.10))
  		numDEgenes_05[i] <- length(which(corr_fdrs[,"Fdr_SAB"] < 0.05))
  		numDEgenes_01[i] <- length(which(corr_fdrs[,"Fdr_SAB"] < 0.01))
...
numDEgenes_per_CT <- as.data.frame(cbind(celltype, numDEgenes_10, numDEgenes_05, numDEgenes_01, numGenes))
colnames(numDEgenes_per_CT) <- c("celltype","numDEgenes_fdr10","numDEgenes_fdr05","numDEgenes_fdr01","num_genes_tested")
write.table(numDEgenes_per_CT, paste0("outputs/",folder,"/3_severity/number_of_DEG_by_CT_qvalues_logFCcutoff.txt"), quote = FALSE)
