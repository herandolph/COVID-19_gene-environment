...
plot_GSEA <- function(geneset, geneset_name){
	...
	## directories
	OUTPUTS_dir <- paste0("outputs/",folder,"/3_severity/")
	...
	readIn <- function(celltype){
		df <- read.table(paste0("inputs/",folder,"/severity_res/",celltype,"_results_with_qvalues.txt"), header = TRUE, sep = ",", check.names = FALSE)
	...
	## probably needs to be geneset specific
	thresh = 2
	...
	merged_pval_m <- reshape2::melt(merged_pval, id.vars = "pathway"); colnames(merged_pval_m)[2] <- "var_padj"; colnames(merged_pval_m)[3] <- "value_padj"
	...
	merged_NES_m <- reshape2::melt(merged_NES, id.vars = "pathway"); colnames(merged_NES_m)[2] <- "var_NES"; colnames(merged_NES_m)[3] <- "value_NES"; merged_NES_m$pathway <- NULL
	...
	merged_subset_m$CT <- factor(merged_subset_m$CT)
	merged_subset_m$CT <- recode_factor(merged_subset_m$CT, "CD14_monocytes" = "CD14_mono", "CD16_monocytes" = "CD16_mono")
	merged_subset_m$CT <- factor(merged_subset_m$CT, levels = c("B","CD4T","CD8T","NK","CD14_mono","CD16_mono"))
	...
	plot <- ggplot(merged_subset_m, aes(x = CT, y = pathway, fill = value_NES_withNA, size = -log10(value_padj), color = CT)) + 
			...
			scale_color_manual(values = rep("black", 7), name = NULL, guide = FALSE) +
	   		...
	   		scale_fill_gradientn(colours = rev(brewer.pal(11,"RdYlBu")), guide = guide_colorbar(frame.colour = "black", ticks.colour = "black", frame.linewidth = 0.3, title = "NES", ticks = FALSE), breaks = breaks_NES, limits = limits_NES, labels = labels_NES, na.value = "grey75") +
			guides(colour = FALSE) + 
	...
	pdf(paste0(OUTPUTS_dir,"/fgsea_heatmaps/",geneset_name,".pdf"), width = 5, height = 5.25, useDingbats=FALSE)
	print(plot)
	dev.off()
}
