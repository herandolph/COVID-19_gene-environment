...
genepos = read.table(paste0("inputs/",folder,"/",genepos_file_name),header = TRUE, stringsAsFactors = FALSE)
colnames(genepos)[1] <- "Gene_ID"
colnames(genepos)[2] <- "chromosome"
## USE SNP POSITIONS FILE FROM THE COVID COHORT ONLY
snpspos = read.table(paste0("inputs/",folder,"/SNP_positions.txt"),header = TRUE, stringsAsFactors = FALSE)
gtypes = read.table(paste0("inputs/",folder,"/genotypes_COVID_and_IAV_merged.txt"), header = TRUE, stringsAsFactors = FALSE, check.names = FALSE)

## create directory structure to save outputs
system(paste0("mkdir -p outputs/",folder,"/",pseudobulk,"/",celltype,"/",temp_dir,"/",condition,"/"))
system(paste0("mkdir -p outputs/",folder,"/",pseudobulk,"/",celltype,"/",condition,"/",out_dir,"/raw_results"))
## create directory for qqplots
system(paste0("mkdir -p outputs/",folder,"/",pseudobulk,"/",celltype,"/",condition,"/",out_dir,"/qqplots"))

## erase any previous files from and re-create the temp file
system(paste0("rm -rf outputs/",folder,"/",pseudobulk,"/",celltype,"/",temp_dir,"/",condition))
system(paste0("mkdir -p outputs/",folder,"/",pseudobulk,"/",celltype,"/",temp_dir,"/",condition))
...
metadata_whole = read.table(paste0("inputs/",folder,"/",metadata_file_name), header = TRUE)
metadata_whole$sample_ID <- as.character(metadata_whole$sample_ID)
## remove individual with no genotyping data (HMN52545)
if(condition == "control"){
    metadata_whole <- metadata_whole[!(metadata_whole$sample_ID %in% "HMN52545_noStim_DSO-NA"), ]
}   

## read in expression data
reads_whole = read.table(paste0("inputs/",folder,"/PSEUDOCOUNTS_MATRICES/",pseudobulk,"/",celltype,"_",counts_file_name), header = TRUE, sep = ",", check.names = FALSE)
...
rownames(metadata_whole) <- metadata_whole$sample_ID
...
metadata_whole$DUP_B2 <- as.character(metadata_whole$DUP_B2)
metadata_whole$DUP_B2[is.na(metadata_whole$DUP_B2)] <- "UNIQUE"
metadata_whole$DUP_B2 <- factor(metadata_whole$DUP_B2)
metadata = metadata_whole[which(metadata_whole$DUP_B2 == "UNIQUE"),]

## keep only certain condition
metadata = metadata[which(metadata$Timepoint==condition),]

## keep only nonstim
metadata = metadata[which(metadata$Stim_Status=="noStim"),]

if(condition == "followup_inf"){
    individuals <- unique(as.character(metadata$individual_ID))

    for(i in 1:length(individuals)){
        indiv_i <- individuals[i]
        metadata_i <- metadata
        metadata_i <- subset(metadata_i, individual_ID %in% indiv_i)
        DSO_max <- max(metadata_i$DSO)
        sample_i <- subset(metadata_i, DSO %in% DSO_max)$sample_ID
        metadata_i <- subset(metadata_i, sample_ID %in% sample_i)

        if(i == 1){
            metadata_new <- metadata_i
        }else{
            metadata_new <- rbind(metadata_new, metadata_i)
        }
    }
...
metadata$Severity_Sampling <- factor(metadata$Severity_Sampling, levels = c("CTL","MODERATE","SEVERE","CRITICAL","FOLLOWUP"))
metadata$RespSupp_Sampling <- factor(metadata$RespSupp_Sampling, levels = c("0","1","2","3","4","-1"))
metadata$Dataset <- factor(metadata$Dataset, levels = c("COVID_B1", "COVID_B2", "IAV"))
metadata$Stim_Status <- factor(metadata$Stim_Status, levels = c("noStim"))
metadata$Age_scale <- scale(metadata$Age)
...
colnames(voomed_reads) = sapply(strsplit(colnames(voomed_reads) ,"_"), `[`, 1)
colnames(voomed_reads) = ifelse(startsWith(colnames(voomed_reads), "CH"), paste0("CR",colnames(voomed_reads)),colnames(voomed_reads))
colnames(voomed_reads) = sub('-', '', colnames(voomed_reads))

rownames(metadata) = sapply(strsplit(rownames(metadata) ,"_"), `[`, 1)
rownames(metadata) = ifelse(startsWith(rownames(metadata), "CH"), paste0("CR",rownames(metadata)),rownames(metadata))
...
metadata = metadata[order(colnames(voomed_reads)),]
...
gtypes_pca = data.frame(snp_id = rownames(gtypes), gtypes, check.names = FALSE)
table(colnames(voomed_reads) %in% colnames(gtypes))
...
snpgdsCreateGeno(paste0("outputs/",folder,"/",pseudobulk,"/",celltype,"/",temp_dir,"/",condition,"/GDS_genotypes.gds"),
...
snpgdsSummary(paste0("outputs/",folder,"/",pseudobulk,"/",celltype,"/",temp_dir,"/",condition,"/GDS_genotypes.gds"))

## load .gds file in as genofile
genofile <- snpgdsOpen(paste0("outputs/",folder,"/",pseudobulk,"/",celltype,"/",temp_dir,"/",condition,"/GDS_genotypes.gds"))
...
counts <- subset(metadata, select = c(paste0(celltype)))
colnames(counts)[1] <- "counts"
metadata <- cbind(metadata, counts)

## add covar table
covariates = t(model.matrix(~ PC1 + PC2 + Dataset + Sex + Age_scale + counts, data = metadata))
covariates <- covariates[rowSums(covariates != 0) > 0, ]
...
snpspos = snpspos[which(snpspos$snp %in% rownames(gtypes)),]
...
length(which(rownames(genotypes)!=snpspos$snp))
table(duplicated(metadata$individual_ID))
...
expression_file_name = paste0("outputs/",folder,"/",pseudobulk,"/",celltype,"/",temp_dir,"/",condition,"/expression.txt")
covariates_file_name = paste0("outputs/",folder,"/",pseudobulk,"/",celltype,"/",temp_dir,"/",condition,"/covariates.txt")
SNP_file_name = paste0("outputs/",folder,"/",pseudobulk,"/",celltype,"/",temp_dir,"/",condition,"/genotypes.txt")
...
permuted_pvalues_folder = paste0("outputs/",folder,"/",pseudobulk,"/",celltype,"/",condition,"/",out_dir,"/raw_results/")
for(iteration in 0:iterations){
  ...
  if(iteration > 0){
    ...
    png(filename =  paste0("outputs/",folder,"/",pseudobulk,"/",celltype,"/",condition,"/",out_dir,"/qqplots/qqplot",iteration,".png"))
...
#### 9. write temporal output files ##########################
    ...
    if(iteration == 0){
        write.table(me$cis$eqtls, file = paste0("outputs/",folder,"/",pseudobulk,"/",celltype,"/",condition,"/",out_dir,"/raw_results/result_original.txt"))}else{
            write.table(me$cis$eqtls, file = paste0("outputs/",folder,"/",pseudobulk,"/",celltype,"/",condition,"/",out_dir,"/raw_results/result_permuted_",iteration,".txt"))}
...
{ 
    if(iteration == 0){
        event = read.table(paste0("outputs/",folder,"/",pseudobulk,"/",celltype,"/",condition,"/",out_dir,"/raw_results/result_original.txt"),header=TRUE)
...
cis_eQTL_fdrs = permFDR(full_data = original_best_EQTL, full_column_id = "pvalue", perm_data = Permutation_Input, perm_column_ids = "all", output_name = paste0("outputs/",folder,"/",pseudobulk,"/",celltype,"/",condition,"/",out_dir))
write.table(cis_eQTL_fdrs$fdrs,file = paste0("outputs/",folder,"/",pseudobulk,"/",celltype,"/",condition,"/",out_dir,"/results_best_SNPs.txt"), quote = FALSE)

## if you want to save the permuted SNPs
permuted_cis_eQTL_fdrs = permFDR(full_data = permuted1_best_EQTL, full_column_id = "pvalue", perm_data = Permutation_Input, perm_column_ids = "all", output_name = paste0("outputs/",folder,"/",pseudobulk,"/",celltype,"/",condition,"/permuted1"))
write.table(permuted_cis_eQTL_fdrs$fdrs, file = paste0("outputs/",folder,"/",pseudobulk,"/",celltype,"/",condition,"/",out_dir,"/permuted1_best_SNPs.txt"), quote = FALSE)

############################################################
#### 12. create qqplot using p-values of best SNPs only ####

results_best_SNPs <- read.table(paste0("outputs/",folder,"/",pseudobulk,"/",celltype,"/",condition,"/",out_dir,"/results_best_SNPs.txt"))

## make and save best SNPs qqplot
png(filename = paste0("outputs/",folder,"/",pseudobulk,"/",celltype,"/",condition,"/",out_dir,"/best_SNPs_qqplot.png"))
...
write.table(x = results_best_SNPs_with_qval, file = paste0("outputs/",folder,"/",pseudobulk,"/",celltype,"/",condition,"/",out_dir,"/results_best_SNPs_with_qval.txt"))

paste0("N SAMPLES = ",nrow(metadata))
